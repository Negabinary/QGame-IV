...
func initialise_world_viewer(time_state:TimeState, world_path:String, actors:Array) -> void:
	...
	columns = nearest_po2(sqrt(world_count))
	rows = world_count / columns
...
func _update_states(time_state:TimeState) -> void:
	...
	for world_id in world_count:
		if state_vector.get_state_squared(world_id) < 0.0000000001:
			...
			world_frames[world_id][0].update_state(time_state, world_id, time_state.get_world_state(world_id))
	...
	for world_id_from in new_time_state.get_world_count():
		...
		if current_vector.get_state_squared(world_id_from)  < 0.0000000001:
			...
			for world_id_to in change_matrix_column:
				...
				world_frame.update_state(new_time_state, world_id_to, world_frame_value_from)
...
func add_frame(row_id:int) -> void:
	...
	world_frame.initialise(loaded_world_frame, actors)
